...
def allocate_param(model_type):
    ...
    # 模型和数据路径
    model_name_or_model_dir = "E:\\kechuang\\yugu\\paraformer\\paraformer_finetune\\model\\speech_paraformer-large_asr_nat-zh-cn-16k-common-vocab8404-online"
    data_dir = "E:\\kechuang\\yugu\\paraformer\\paraformer_finetune\\mydata\\list"
    train_data = f"{data_dir}\\train.jsonl"
    val_data = f"{data_dir}\\val.jsonl"

    train_src_path = r"E:\kechuang\yugu\paraformer\paraformer_finetune\mydata\list\train_wav.scp"
    train_dst_path = r"E:\kechuang\yugu\paraformer\paraformer_finetune\mydata\list\train_text.txt"

    # 生成 .jsonl 数据文件
    # subprocess.run([
    #     "scp2jsonl",
    #     f"++scp_file_list=['{train_src_path}', '{train_dst_path}']",
    #     "++data_type_list=['source', 'target']",
    #     f"++jsonl_file_out={train_data}"
    # ], check=True)
    #
    # val_src_path = r"E:\kechuang\yugu\paraformer\paraformer_finetune\mydata\list\val_wav.scp"
    # val_dst_path = r"E:\kechuang\yugu\paraformer\paraformer_finetune\mydata\list\val_text.txt"
    #
    # subprocess.run([
    #     "scp2jsonl",
    #     f"++scp_file_list=['{val_src_path}', '{val_dst_path}']",
    #     "++data_type_list=['source', 'target']",
    #     f"++jsonl_file_out={val_data}"
    # ], check=True)

    # 实验输出路径和日志文件
    output_dir = "E:\\kechuang\\yugu\\paraformer\\paraformer_finetune\\finetune_run"
    log_file = f"{output_dir}\\log.txt"
    # deepspeed_config = os.path.join(workspace, "..", "..", "ds_stage1.json")
    ...
    #     cfg = compose(config_name=None, overrides=[f'cfg={cfg}'])

    return cfg


if __name__ == "__main__":
    cfg = allocate_param(model_type="paraformer-streaming")
    main_hydra(cfg)
